...
class XmlToLst(object):
    ...
    def __init__(self, out_dir, lst_name, custom_classes, multi_slices=False):
    ...
    def start_parse(self, img_folder, label_folder):
        if os.path.isdir(label_folder):
            if self.multi_slices:
                self.dcm_slices2lst(self.out_dir, img_folder, label_folder, self.lst_name)
            else:
        ...
        with open(lst_file, 'w') as f:
            for base_dir, _, file_iter in os.walk(label_folder):
                for targ_file in fnmatch.filter(file_iter, "*" + XML_SUFFIX):
                    ...
                    folder_content = tree.find('folder').text
                    if folder_content:
                        fname = os.path.join(folder_content, tree.find('filename').text)
                    else:
    ...
    def dcm_slices2lst(self, out_dir, img_folder, label_folder, lst_name):
        XML_SUFFIX = '.xml'
        LABEL_SUFFIX = '.lst'
        lst_file = os.path.join(out_dir, lst_name + LABEL_SUFFIX)
        fnum = 0
        ...
        with open(lst_file, 'w') as f:
            for base_dir, _, file_iter in os.walk(label_folder):
                for targ_file in fnmatch.filter(file_iter, "*" + XML_SUFFIX):
                    label_file = os.path.join(base_dir, targ_file)
                    tree = xml.etree.ElementTree.parse(label_file)
                    origin_file = tree.find('filename').text
                    folder_content = tree.find('folder').text
                    if folder_content:
                        fname = os.path.join(tree.find('folder').text, origin_file)
                    else:
                        fname = origin_file
                    # Series need to be a folder
                    if not os.path.isdir(os.path.join(img_folder, origin_file.split('_')[0])):
                        # process may not be continuous
                        continue
                    size = tree.find('size')
                    width = size.find('width').text
...
parser.add_argument('--multi-slices', action='store_true',
                    help='Whether parse multi-slices dcm images')
opt = parser.parse_args()


def main():
    custom_classes = opt.custom_classes.split(',') if opt.custom_classes else opt.custom_classes
    xml2lst = XmlToLst(opt.out_dir, opt.lst_name, custom_classes, opt.multi_slices)
